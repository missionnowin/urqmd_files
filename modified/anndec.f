...
c           if(i1.eq.23.and.i.eq.23)then 
c           write(*,*)i1,i,m1,prob(i)
c           stop
c           endif
 3       continue
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       if(ir.eq.23.and.i.eq.27.and.em.le.4.671)then
           fbrancx=0.0d0
           return
       endif
...
c       if(i.eq.23.and.ir.eq.23)then
c        write(*,*)em,bi
c        stop
c        endif    

c       if((ires.gt.1).and.(ires.lt.maxdel))then
c          if(i.eq.14)then
c             fbrancx=bi
c             return
c          endif
...
cJS   alpha is now calculated according to the Manley fit extended for decays with l>2
cJS   alpha=(q0/Lambda/**(2l) where q0 is the average CM momentum at the
cJS   pole and Lambda=1/fm**2

         lamman=pmean(mn,b1,isoit(b1),b2,isoit(b2),
     &                       b3,isoit(b3),b4,isoit(b4),ipwr)

         alph=lamman/(0.1973)**ipwr
         b=bi*mn/em*kdiv1*(1.0d0+alph)/(1.+alph*kdiv2)           
