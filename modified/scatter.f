...
c         if(acttime.le.0.2d0)then
c            if((abs(xtotfac(ind1)-CTParam(74)).le.0.00001d0)
c     $           .or.(abs(xtotfac(ind2)-CTParam(74))
c     $           .le.0.00001d0))then
c               sigfac=0.0d0
c            endif
c         endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine scatFinal(colldens,sqrts)
c
c     Revision : 1.0
c
...
c     number of collisions
         ...
         if(ctoption(67).eq.1)then
         if(((iline.eq.17).or.(iline.eq.19)).and.sqrts.lt.1.95d0)then
          ncoll(inew(i))=ncoll(inew(i))-1
         endif
         endif
...
C...ROTATE 
        ...
        ROT(3,3)=COS(THE)
        DO J=1,3
           P(J)=ROT(J,1)*P1+ROT(J,2)*P2+ROT(J,3)*P3
        END DO
        p(4)=p4
      else
        p(1)=p1
...
c JS now set by CTParam(74)
       do 1 ll=n1l,n2l
         leadfac(ll)=CTParam(74)
 1    continue
