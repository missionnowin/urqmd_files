...
c.. pythia particle list
      data pytlist/1,-1,41,54,63,69,101/
...
c     JS Maximum allowed mass is mresmax
c     JS Mass distribution is dirty fix and 
c     can/should be done much better

      mmax7=min(mresmax,e-1.077d0)-1.077d0
c      m3=fmsr(mminit(mindel),mmax7)
      m3=1.077d0+(ranf(0)*mmax7)
      mmaxdis7=dexp(-(1.5d0-m3)**2/0.5d0)
      if(ranf(0).gt.mmaxdis7)goto 7

 935  continue
     
      mmax7=min(mresmax,e-m3)-1.077d0    
c      m4=fmsr(mminit(mindel),mmax7)
      m4=1.077d0+(ranf(0)*mmax7)
      mmaxdis7=dexp(-(1.5d0-m4)**2/0.5d0)
      if(ranf(0).gt.mmaxdis7)goto 935

      if(bit)i3=whichres(m3,5)
      if(bit)i4=whichres(m4,5)      
      if(ranf(0).gt.0.5d0)call swpizm(i3,iz3,m3,i4,iz4,m4)
      goto 1008
...
c id's of the lowest multiplet (with the same quark content)
        ...
        call id2ityp(iddum,0d0,i1,iz1)
         ...
         call getirg(abs(i1),i1m,i1p)
         i1=isign(i1m,i1)
c         if(i1old.lt.0)write(*,*)'getirg1', i1,i1old,i1m,i1p
      endif
...
c same for second particle
      if(iabs(i2).ge.minmes)then
        ...
        call id2ityp(iddum,0d0,i2,iz2)
         ...
         call getirg(abs(i2),i2m,i2p)
         i2=isign(i2m,i2)
c         if(i2old.lt.0)write(*,*)'getirg1', i2,i2old,i2m,i2p
      endif
...
c     hp flag is necessary to fix angular distribution         
      pythflag=1
...
c mass excitation according 1/m
      ...
      if((ms1.ne.ms1).or.(ms2.ne.ms2))then
         write(*,*)"error in make22 - string",
     $        ms1, ms2,i1,i2,e,i1old,i2old
         stop
      endif
...
c K- + p -> Y + pi
     ...
     &   i2.eq.-106.and.iz2.eq.-1) then   !K-
         sig1 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,101,-2) !Sigma+ + Pi-
         sig2 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,-2,101,2) !Sigma- + Pi+
         sig3 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,0)  !Lambda0/Sigma0 + Pi0
         call getbran((/sig1,sig2,sig3/),1,3,rdummy,1,3,channel)
         i3=minsig
         i4=101
         select case(channel)
           case(1) !Sigma+ + Pi-
...
c K- + n -> Y + pi
     ...
     &   i2.eq.-106.and.iz2.eq.-1) then    !K-
        sig1 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,-2,101,0) !Sigma- + Pi0
        sig2 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,-2) !Lambda0/Sigma0 + Pi-
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        i3=minsig
        i4=101
        select case(channel)
          case(1) !Sigma- + Pi0
...
c K+ + ap -> aY + pi
     ...
     &   i2.eq.106.and.iz2.eq.1) then      !K+
        sig1 = STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,2,101,-2) !aSigma+ + Pi-
        sig2 = STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,-2,101,2) !aSigma- + Pi+
        sig3 = STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,101,0)  !aLambda0/aSigma0 + Pi0
        call getbran((/sig1,sig2,sig3/),1,3,rdummy,1,3,channel)
        i3=-minsig
        i4=101
        select case(channel)
...
c K+ + an -> aY + pi
     ...
     &   i2.eq.106.and.iz2.eq.1) then     !K+
        sig1 = STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,2,101,0)  !aSigma+ + Pi0
        sig2 = STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,101,2)  !aLambda0/aSigma0 + Pi+
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        i3=-minsig
        i4=101
        select case(channel)
...
c aK0 + p
     ...
     &   i2.eq.-106.and.iz2.eq.1)  then   !aK0
        sig1 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,101,0) !Sigma+ + Pi0
        sig2 = STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,2) !Lambda0/aSigma0 + Pi+
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        i3=minsig
        i4=101
        select case(channel)
...
c and 
c Sigma- + pi+   -> K-+p  & aK0+n
      if(i1.eq.minsig.and.abs(iz1).eq.2.and.       !charged Sigma
...
c Sigma0 + pi0   -> K-+p  & aK0+n
      elseif(i1.eq.minsig.and.iz1.eq.0.and.        !Sigma0
...
c Lambda0 + pi0  -> K-+p  & aK0+n
      elseif(i1.eq.minlam.and.iz1.eq.0.and.        !Lambda0
     &       i2.eq.101.and.iz2.eq.0) then      !pi0
        i3=1
        i4=-106
        sig1 = STREXCHANGE(e,1,1,-106,-1,i1,iz1,i2,iz2)  !K- + p
...
c and
c aSigma- + pi+  -> K++ap & K0+an
      elseif(i1.eq.-minsig.and.abs(iz1).eq.2.and.      !charged aSigma
     &       i2.eq.101.and.abs(iz2).eq.2.and.      !charged Pi
     &       iz1+iz2.eq.0) then                    !total isospin=0
         i3=-1
         i4=106
...
c aSigma0 + pi0  -> K++ap & K0+an
      elseif(i1.eq.-minsig.and.iz1.eq.0.and.       !aSigma0
     &       i2.eq.101.and.iz2.eq.0) then      !pi0
        i3=-1
        i4=106
        sig1 = STREXCHANGE(e,-1,-1,106,1,i1,iz1,i2,iz2)  !K+ + ap
...
c aLambda0 + pi0 -> K++ap & K0+an
      elseif(i1.eq.-minlam.and.iz1.eq.0.and.       !aLambda0
     &       i2.eq.101.and.iz2.eq.0) then      !pi0
        i3=-1
        i4=106
        sig1 = STREXCHANGE(e,-1,-1,106,1,i1,iz1,i2,iz2)  !K+ + ap
...
c-----positive---------
c Sigma+ + pi0   -> aK0+p
      elseif(i1.eq.minsig.and.iz1.eq.2.and.      !Sigma+
     &       i2.eq.101.and.iz2.eq.0) then    !pi0
        i3=1
        iz3=1
        i4=-106
...
c Lambda0 + pi+  -> aK0+p
     ...
     &       i2.eq.101.and.iz2.eq.2) then    !pi+
        ...
        iz4=1
c aSigma+ + pi0  -> K++an
      elseif(i1.eq.-minsig.and.iz1.eq.2.and.     !aSigma+
     &       i2.eq.101.and.iz2.eq.0) then    !pi0
        i3=-1
        iz3=1
        i4=106
...
c Sigma+ + K- -> Xi0 + pi0
c             -> Xi- + pi+
      if(i1.eq.minsig.and.iz1.eq.2.and.        !Sigma+
     &   i2.eq.-106.and.iz2.eq.-1) then    !K-
        sig1=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,0)
        sig2=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,101,2)
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        i3=mincas
        select case(channel)
          case(1)
...
c Sigma+ + aK0-> Xi0 + pi+
      else if(i1.eq.minsig.and.iz1.eq.2.and.   !Sigma+
     &   i2.eq.-106.and.iz2.eq.1) then     !aK0
        i3=mincas
        iz3=1
        iz4=2
...
c Lambda0/Sigma0 + aK0 -> Xi- + pi+
      else if((i1.eq.minsig.or.i1.eq.minlam).and.iz1.eq.0.and.  !Lambda0/Sigma0
     &   i2.eq.-106.and.iz2.eq.1) then                  !aK0
        sig1=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,0) 
        sig2=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,101,2)
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
...
c Lambda0/Sigma0 + K-  -> Xi- + pi0
     ...
     &   i2.eq.-106.and.iz2.eq.-1) then                 !aK-
        sig1=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,-2) 
        sig2=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,101,0)
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        i3=mincas
        select case(channel)
          case(1)
...
c aSigma+ + K0 -> aXi+ + pi0
c              -> aXi0 + pi+
      else if(i1.eq.-minsig.and.iz1.eq.2.and. !aSigma+
     &   i2.eq.106.and.iz2.eq.-1) then    !K0
        sig1=STREXCHANGE(e,i1,iz1,i2,iz2,-mincas,1,101,0)
        sig2=STREXCHANGE(e,i1,iz1,i2,iz2,-mincas,-1,101,2)
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
...
c aLambda0/aSigma0 + K0 -> aXi+ + pi-
      else if((i1.eq.-minsig.or.i1.eq.-minlam).and.iz1.eq.0.and.  !aLambda0/aSigma0
     &   i2.eq.106.and.iz2.eq.-1) then                    !K0
        sig1=STREXCHANGE(e,i1,iz1,i2,iz2,-mincas,-1,101,0) 
        sig2=STREXCHANGE(e,i1,iz1,i2,iz2,-mincas,1,101,-2)
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
...
c Xi0 + pi0 -> aK0+ Lambda0/Sigma0
      if(i1.eq.mincas.and.iz1.eq.1.and.        !Xi0
     &   i2.eq.101.and.iz2.eq.0) then      !pi0
        sig1=STREXCHANGE(e,minsig,2,-106,-1,i1,iz1,i2,iz2)
     *      *pcms(e,massit(-106),massit(minsig))**2
     *      /pcms(e,massit(i1),massit(i2))**2
        sig2=STREXCHANGE(e,minsig,0,-106,1,i1,iz1,i2,iz2)
     *      *pcms(e,massit(-106),massit(minsig))**2
     *      /pcms(e,massit(i1),massit(i2))**2
        sig3=STREXCHANGE(e,minlam,0,-106,1,i1,iz1,i2,iz2)
     *      *pcms(e,massit(-106),massit(minlam))**2
     *      /pcms(e,massit(i1),massit(i2))**2
        call getbran((/sig1,sig2,sig3/),1,3,rdummy,1,3,channel)
        select case(channel)
          case(1)
...
c Xi0 + pi- -> aK0+ Sigma-
      elseif(i1.eq.mincas.and.iz1.eq.1.and.    !Xi0
     &   i2.eq.101.and.iz2.eq.-2) then     !pi-
        i4 =-106
        sig1=STREXCHANGE(e,minsig,0,-106,-1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(minsig))**2
     &      /pcms(e,massit(i1),massit(i2))**2
        sig2=STREXCHANGE(e,minlam,0,-106,-1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(minlam))**2
     &      /pcms(e,massit(i1),massit(i2))**2
        sig3=STREXCHANGE(e,minsig,-2,-106,1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(minsig))**2
     &      /pcms(e,massit(i1),massit(i2))**2
        call getbran((/sig1,sig2,sig3/),1,3,rdummy,1,3,channel)
        select case(channel)
...
c Xi- + pi+ -> K-  + Sigma+
      elseif(i1.eq.mincas.and.iz1.eq.-1.and.   !Xi-
     &   i2.eq.101.and.iz2.eq.2) then      !pi+
        i4 =-106
        sig1=STREXCHANGE(e,minsig,0,-106,1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(minsig))**2
...
c aXi+ + pi0 -> K+ + aLambda0/aSigma0
      elseif(i1.eq.-mincas.and.iz1.eq.1.and.   !aXi+
     &   i2.eq.101.and.iz2.eq.0) then      !pi0
        sig1=STREXCHANGE(e,-minsig,2,106,-1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(106),massit(-minsig))**2
     &      /pcms(e,massit(i1),massit(i2))**2
...
c Xi0 + aK0 -> Omega- + pi+
      if(i1.eq.mincas.and.iz1.eq.1.and.
...
c Xi0 + K- -> Omega- + pi0
c Xi- + aK0 -> Omega- + pi0
      elseif((i1.eq.mincas.and.iz1.eq.1.and.
     &   i2.eq.-106.and.iz2.eq.-1).or.
     &  (i1.eq.mincas.and.iz1.eq.-1.and.
     &   i2.eq.-106.and.iz2.eq.1)) then
        i3  = minome
        iz3 = 0
        i4  = 101
...
62    continue
c Omega- + pi+ -> Xi0 + aK0
      if(i1.eq.minome.and.iz1.eq.0.and.
...
c Omega- + pi0 -> Xi0 + K-
c Omega- + pi0 -> Xi- + aK0
      elseif(i1.eq.minome.and.iz1.eq.0.and.
     &   i2.eq.101.and.iz2.eq.0) then
        i3  = mincas
        i4  = -106
        sig1=5d-1*STREXCHANGE(e,mincas,1,-106,-1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(mincas))**2
     &      /pcms(e,massit(i1),massit(i2))**2
        sig2=5d-1*STREXCHANGE(e,mincas,-1,-106,1,i1,iz1,i2,iz2)
     &      *pcms(e,massit(-106),massit(mincas))**2
     &      /pcms(e,massit(i1),massit(i2))**2
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        select case(channel)
...
c                   -> Xi- + p
     ...
     &    (i1.eq.minlam.or.i1.eq.minsig).and.
     &    (i2.eq.minlam.or.i2.eq.minsig)).or. 
     &   ((i1.eq.minsig.and.i2.eq.minsig).and.   !Sigma+ + Sigma-
...
c Sigma0  + Sigma+  -> Xi0 + p
      elseif((iz1.eq.0.and.iz2.eq.2.and.i1.eq.minlam.and.i2.eq.minsig)
     $       .or.
     &     (iz1.eq.2.and.iz2.eq.0.and.i1.eq.minsig.and.i2.eq.minlam).or.
     &   (i1.eq.minsig.and.i2.eq.minsig.and.((iz1.eq.2.and.iz2.eq.0).or.
...
c aSigma0  + aSigma-  -> aXi0 + ap
      elseif((iz1.eq.0.and.iz2.eq.-2.and.i1.eq.-minlam.and.i2.eq.
     $       -minsig).or.
     &  (iz1.eq.-2.and.iz2.eq.0.and.i1.eq.-minsig.and.i2.eq.-minlam).or.
     &(i1.eq.-minsig.and.i2.eq.-minsig.and.((iz1.eq.-2.and.iz2.eq.0).or.
     &                                   (iz1.eq.0.and.iz2.eq.-2))))then
...
c Xi- + p -> Sigma+  + Sigma-
     ...
     &    si2.eq.mincas.and.siz2.eq.1).or.
     &   (si1.eq.1.and.siz1.eq.1.and.
     &    si2.eq.mincas.and.siz2.eq.-1)) then
        sig1=STREXCHANGE(e,si1,siz1,si2,siz2,minlam,0,minlam,0) !Lam + Lam
        sig2=STREXCHANGE(e,si1,siz1,si2,siz2,minlam,0,minsig,0) !Lam + Sig0
        sig3=STREXCHANGE(e,si1,siz1,si2,siz2,minsig,0,minsig,0) !Sig0 + Sig0
        sig4=STREXCHANGE(e,si1,siz1,si2,siz2,minsig,2,minsig,-2)!Sig+ + Sig-
        call getbran((/sig1,sig2,sig3,sig4/),1,4,rdummy,1,4,channel)
        select case(channel)
          case(1)
            i3  = minlam
            iz3 = 0
...
c Xi0 + p -> Sigma0  + Sigma+
     ...
     &       si2.eq.mincas.and.siz2.eq.1) then
        sig1=STREXCHANGE(e,si1,siz1,si2,siz2,minlam,0,minsig,2) !Lam + Sig+
        sig2=STREXCHANGE(e,si1,siz1,si2,siz2,minsig,0,minsig,2) !Sig0 + Sig+
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        select case(channel)
          case(1)
            i3  = minlam
...
c aXi+ + ap -> aSigma-  + aSigma+
     ...
     &       (si1.eq.-1.and.siz1.eq.-1.and.
     &        si2.eq.-mincas.and.siz2.eq.1)) then
        sig1=STREXCHANGE(e,si1,siz1,si2,siz2,-minlam,0,-minlam,0)  !aLam + aLam
        sig2=STREXCHANGE(e,si1,siz1,si2,siz2,-minlam,0,-minsig,0) !aLam + aSig0
        sig3=STREXCHANGE(e,si1,siz1,si2,siz2,-minsig,0,-minsig,0)  !aSig0 + aSig0
        sig4=STREXCHANGE(e,si1,siz1,si2,siz2,-minsig,-2,-minsig,2) !aSig- + aSig+
        call getbran((/sig1,sig2,sig3,sig4/),1,4,rdummy,1,4,channel)
        select case(channel)
          case(1)
            i3  = -minlam
...
c aXi0 + ap -> aSigma0  + aSigma+
      elseif(si1.eq.-1.and.siz1.eq.-1.and.
     &       si2.eq.-mincas.and.siz2.eq.-1) then
        sig1=STREXCHANGE(e,si1,siz1,si2,siz2,-minlam,0,-minsig,-2) !aLam + aSig-
        sig2=STREXCHANGE(e,si1,siz1,si2,siz2,-minsig,0,-minsig,-2) !aSig0 + aSig-
        call getbran((/sig1,sig2/),1,2,rdummy,1,2,channel)
        select case(channel)
          case(1)
            i3  = -minlam
...
c              nd    nn*    nd*   dd      dn*    dd*    B*B*
      data ar/ 3.3d4,  4.7d0, 10.0d0, 4.0d0,  1.0d0, 2.0d0, 0.045d0 /
...
c calculate cross section
     ...
     &       *dble((jit(im)+1)*(jit(jm)+1))
         ...
         gaussadd=7.0d0*dexp(-(e-2.138d0)**2/0.002d0)
c     $        +3.0d0*dexp(-(e-2.68d0)**2/0.07d0)
         fppfit=fppfit+gaussadd
...
cJS            iim=2
         ...
         if(io.ge.4.and.e.le.2.154d0)then
           fppfit=0.0d0
           return
         endif
...
c     calculate cross section
     ...
     &  /((massit(jm)-massit(im))**2*(massit(jm)+massit(im))**2)
         ...
         if(io.eq.4.and.e.gt.2.155d0)then
           fppfit=fppfit+3.0d0*dexp(-(e-2.68d0)**2/0.07d0)
         endif
...
c sofar set to zero
      if(wtabflg.ge.3.and.CTOption(9).eq.0)then
c     table lookup
c         if(im.eq.mindel)then
c            write(*,*)
c     iim=im
c     elseif(im.eq.mindel)then
c     iim=2
c     else
c     write(*,*)'(E) fppfit: First particle should be N or Delta'
c     stop 137
c         endif
         if((im.gt.1).and.(jm.gt.1).and.(im.ne.mindel)
     $        .and.(jm.ne.mindel))then
            
            fppfit=max(0d0,splint(tabxnd,frrtaby(1,1,im,jm),
     .           frrtaby(1,2,im,jm),widnsp,e))
         else
            fppfit=0.0d0
         endif

c         if(im.eq.mindel)then
c            write(*,*)fppfit,im,jm
c         endif
...
c     calculate cross section
     ...
     &        *dble((jit(im)+1)*(jit(jm)+1))
         ...
         if(im.ne.jm) then
c     write(*,*)"B*B*1",im,jm,tmp,massit(jm),massit(im)
            tmp=tmp
     &    /(1.0d0+(massit(jm)+massit(im))**2*(massit(jm)-massit(im))**2)
         endif

         fppfit=tmp
c         fppfit=0.0d0
      endif  
c      write(*,*)"B*B*",im,jm,fppfit
...
c...functions
      ...
      real*8 STREXWrapper,sigel,sigel2
...
cJS  no inelastic pp scattering
      if(ctoption(61).eq.1)then
      if(io.le.7.or.io.eq.15)then         
         sig=0.0d0
         return
      endif
      if(io.eq.19)then
...
c     JS no Lambda_c scattering yet
      if(io.ne.20.and.(iabs(ii1).eq.70.or.iabs(ii2).eq.70))then
         sig=0.0d0
         return
      endif
...
c JS  separate parametrization for Y-B scattering
c     (should be checked if also useful for S>1?)
      if(abs(i1).ge.41.or.abs(i2).ge.41)then
       if(e.ge.5.6d0)then
         sigel=5.0d0
       else
        sigel=-2.46716d0+170.36819d0*dexp(-(e-2.05718d0)/0.00788d0)
     &  +30.60191d0*dexp(-(e-2.05718d0)/0.00232d0)
     &  +15.11224d0*dexp(-(e-2.05718d0)/5.02811d0)
       endif
       sigel2=min(sigel,150.0d0)
       sigel=28.48335d0-25167600.d0*exp(-e/0.19669d0)
       sig=max(sigel,0.0d0)
       sig=sig+sigel2
      endif
...
c     Lambda+Lambda Strangness-Exchange Cross-section
      if(CTOption(59).gt.1) then
        if((abs(i1).eq.minlam.and.abs(i2).eq.minlam.and.i1*i2.gt.0).or.
c       Lambda+Sigma Strangness-Exchange Cross-section
     &     (((abs(i1).eq.minlam.and.abs(i2).eq.minsig).or.
     &       (abs(i1).eq.minsig.and.abs(i2).eq.minlam)).and.
     &             i1*i2.gt.0).or.
c       Sigma+Sigma Strangness-Exchange Cross-section
     &       (abs(i1).eq.minsig.and.abs(i2).eq.minsig.and.
     &           i1*i2.gt.0)) then
          sig=sig+STREXWrapper(63,e,i1,iz1,i2,iz2)
c       Xi + Nucleon Strangness-Exchange Cross-section
        elseif(((abs(i1).eq.mincas.and.abs(i2).eq.1).or.
     &          (abs(i1).eq.1.and.abs(i2).eq.mincas)).and.
     &             i1*i2.gt.0) then
...
c     (should be checked if also useful for S>1?)
     ...
     &  +15.11224d0*dexp(-(e-2.05718d0)/5.02811d0)
       ...
       sig=min(sigel,150.0d0)
      endif


c      if(e.lt.200.0d0.and.abs(i1).lt.41.and.abs(i2).lt.41)then
c JS  Power law fit towards low energies similar to p+p
c      sig=3.49d0*e**(0.19d0)
c      endif
...
c...bbar total...from ppbar via AQM & phase-space
      ...
      if(ctoption(63).ge.4)then
              sig=sappela(e,m1,m2)*bbphi/nnphi+40.0d0
      endif
      return
...
c For Kaon+N reactions this is already done in nonres.
         if((abs(i1).eq.mincas.or.abs(i1).eq.minlam).and.
     &       abs(i2).eq.pimeson) then
           sig=sig+STREXWrapper(58,e,i1,iz1,i2,iz2)
         elseif((abs(i1).eq.minsig.or.abs(i1).eq.minlam).and.
     &           abs(i2).eq.itkaon) then
           sig=sig+STREXWrapper(59,e,i1,iz1,i2,iz2)
         elseif(abs(i1).eq.mincas.and.abs(i2).eq.pimeson) then
           sig=sig+STREXWrapper(60,e,i1,iz1,i2,iz2)
         elseif(abs(i1).eq.mincas.and.abs(i2).eq.106) then
           sig=sig+STREXWrapper(61,e,i1,iz1,i2,iz2)
         elseif(abs(i1).eq.minome.and.abs(i2).eq.101) then
...
c     JS No strings from charm
      if((abs(i1).ge.133).or.(abs(i2).ge.133))then
         sig=0.0d0
         return
      endif 
...
c JS: this threshold value could be moved up some time.
...
c-- minus elastic x-sect. (scaled with el. aqm)
      ...
      sig=sig-sighera(m2,m1,e,8)*bbphi/nnphi
c-- melting
c JS: The following is a possible fix to allow for inelastic scattering
c in K+ + p below 3GeV! Will create some problems in 2 Stringexcitation
c      qflg=0.d0
c      do kq1=1,2
c       do kq2=1,3
c JS: two of the quarks must be able to annihilate, 
c JS: if no s-channels is possible, always use 2-string down to 1.7GeV:
c        if(iq1(kq1)+iq2(kq2).eq.0) qflg=1.d0
c       enddo
c      enddo

c      if(qflg.eq.0)then
c      sig=xmelt(e,0d0,sig,1.7d0,4.d0)
c      else
...
c    cross sections
      call aqm(i1,i2,sig,dum)      
      sig=sig-dum
      sig=sig*bcms(e,max(mminit(i1),m1),max(mminit(i2),m2))**
     $     (aaqm*2.5d0)
...
cJS   Multiply with 1/2 for equal particles in final state or different
c     particles in np->ND
...
c x_string+x_resonances
          sig=sig*bcms(e,mminit(i1),mminit(i2))**(aaqm*2.5d0)
c get x_string
...
c Xi + pi -> Sigma/Lambda + K
      sig=0d0
      if(CTOption(59).lt.1) return
      if(e.le.massit(minsig)+massit(106)) return !enough energy for Sig/Lam + K
      sig=STREXWrapper(io,e,i1,iz1,i2,iz2)
      return
...
c Xi + K -> Omega + pi
      sig=0d0
      if(CTOption(59).lt.1) return
      if(e.le.massit(minome)+massit(101)) return !enough energy for Omega + pi
      sig=STREXWrapper(io,e,i1,iz1,i2,iz2)
      return
...
c Lambda/Sigma + Lambda/Sigma -> Xi + N
      sig=0d0
      if(e.le.massit(mincas)+massit(1)) return !enough energy for Xi + N
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      if((abs(i1).ne.1.and.abs(i1).ne.minlam.and.abs(i1).ne.minsig
     &                                  .and.abs(i1).ne.mincas).or.
     &    abs(iz1).gt.2.or.                   !Nucleon/Lambda/Sigma/Xi

     &   (abs(i2).ne.106.and.abs(i2).ne.minlam.and.abs(i2).ne.minsig
     &                  .and.abs(i2).ne.1 .and.abs(i2).ne.mincas).or.
     &   abs(iz2).gt.2.or.                    !Kaons/Nucleon/Lambda/Sigma/Xi
     &  (abs(i3).ne.minlam.and.abs(i3).ne.minsig.and.abs(i3).ne.mincas
     &                 .and.abs(i3).ne.1.and.abs(i3).ne.minome).or.
     &   abs(iz3).gt.2.or.                    !Nucleons/Lambda/Sigma/Xi/Omega
     &   (abs(i4).ne.101.and.abs(i4).ne.1.and.abs(i4).ne.mincas 
     &                 .and.abs(i4).ne.minlam.and.abs(i4).ne.minsig).or.
...
c K- + p -> Y + pi
      if(i1.eq.1.and.iz1.eq.1.and.       !proton  
     &   i2.eq.-106.and.iz2.eq.-1) then  !K-
        if(i3.eq.minsig.and.iz3.eq.2.and.                   !Sigma+
     &     i4.eq.101.and.iz4.eq.-2)then                 !Pi-
           sig=0.0788265d0/(e-1.38841)**2
           if(sig.gt.38) sig=38d0
        elseif(i3.eq.minsig.and.iz3.eq.-2.and.              !Sigma-
     &     i4.eq.101.and.iz4.eq.2) then                 !Pi+
           sig=0.0396741d0/(e-1.41)**2
           if(sig.gt.100) sig=100d0
        elseif((i3.eq.minsig.or.i3.eq.minlam).and.iz3.eq.0.and. !Sigma0/Lambda0
...
c K- + n -> Y + pi
     ...
     &   i2.eq.-106.and.iz2.eq.-1) then   !K-
        if(i3.eq.minsig.and.iz3.eq.-2.and. 
     &     i4.eq.101.and.iz4.eq.0) then
           !same as 0.5*((K-p->S-pi+)+(K-p->S+pi-))
           sig=5d-1*(STREXCHANGE(e,i1,-iz1,i2,iz2,minsig,-2,101,2)
     &             + STREXCHANGE(e,i1,-iz1,i2,iz2,minsig,2,101,-2))
        elseif((i3.eq.minsig.or.i3.eq.minlam).and.iz3.eq.0.and.
     &     i4.eq.101.and.iz4.eq.-2) then
           sig=STREXCHANGE(e,i1,-iz1,i2,iz2,minsig,0,101,0) !equal to K-+p->S0/L0+pi0
        endif
c K+ + ap -> aY + pi
      else if (i1.eq.-1.and.iz1.eq.-1.and. !aProton
     &   i2.eq.106.and.iz2.eq.1) then   !K+
        if(i3.eq.-minsig.and.iz3.eq.2.and.
     &      i4.eq.101.and.iz4.eq.-2) then
          sig=STREXCHANGE(e,-i1,-iz1,-i2,-iz2,minsig,-2,101,2) !anti reaction
        elseif(i3.eq.-minsig.and.iz3.eq.-2.and.
     &    i4.eq.101.and.iz4.eq.2) then
          sig=STREXCHANGE(e,-i1,-iz1,-i2,-iz2,minsig,2,101,-2) !anti reaction
        elseif((i3.eq.-minsig.or.i3.eq.-minlam).and.iz3.eq.0.and.
     &    i4.eq.101.and.iz4.eq.0) then
          sig=STREXCHANGE(e,-i1,-iz1,-i2,-iz2,minsig,0,101,0)  !anti reaction
        endif
c K+ + an -> aY + pi
      else if (i1.eq.-1.and.iz1.eq.1.and. !aNeutron
     &   i2.eq.106.and.iz2.eq.1) then     !K+
        if(i3.eq.-minsig.and.iz3.eq.2.and. 
     &     i4.eq.101.and.iz4.eq.0) then
           sig=STREXCHANGE(e,-i1,-iz1,-i2,-iz2,minsig,-2,101,0)  !anti reaction
        elseif((i3.eq.-minsig.or.i3.eq.-minlam).and.iz3.eq.0.and.
     &     i4.eq.101.and.iz4.eq.2) then
           sig=STREXCHANGE(e,-i1,-iz1,-i2,-iz2,minsig,0,101,-2)   !anti reaction
        endif
c K0 + ap
      else if (i1.eq.-1.and.iz1.eq.-1.and.!aProton
     &   i2.eq.106.and.iz2.eq.-1)  then   !K0
...
c aK0 + p
     ...
     &     i4.eq.101.and.iz4.eq.0) then
          ...
          sig=STREXCHANGE(e,i1,-iz1,i2,-iz2,minsig,-2,101,0)
        elseif((i3.eq.minsig.or.i3.eq.minlam).and.iz3.eq.0.and.
     &     i4.eq.101.and.iz4.eq.2) then  
          !same as K- + n -> Lambda0/Sigma0 + pi-
          sig=STREXCHANGE(e,i1,-iz1,i2,-iz2,minsig,0,101,-2)
        endif
c aK0 + n
      else if (i1.eq.1.and.iz1.eq.-1.and. !neutron
     &   i2.eq.-106.and.iz2.eq.1)  then   !aK0
        if(i3.eq.minsig.and.iz3.eq.2.and.                  
     &    i4.eq.101.and.iz4.eq.-2)then                
          sig=STREXCHANGE(e,1,1,-106,-1,minsig,2,101,-2) !same as K-+p -> Sigma+ + pi-
        elseif(i3.eq.minsig.and.iz3.eq.-2.and.           
     &    i4.eq.101.and.iz4.eq.2) then              
          sig=STREXCHANGE(e,1,1,-106,-1,minsig,-2,101,2) !same as K-+p -> Sigma- + pi+
        elseif((i3.eq.minsig.or.i3.eq.minlam).and.iz3.eq.0.and. 
     &    i4.eq.101.and.iz4.eq.0) then             
          sig=STREXCHANGE(e,1,1,-106,-1,minsig,0,101,0)  !same as K-+p -> Sigma0 + pi0
        end if



c aK + Sigma
      else if(i1.eq.minsig.and.i2.eq.-106) then
        if(iz1.eq.2.and.iz2.eq.1.and.                !Sig+ + aK0 -> Xi0 + pi+
     &     i3.eq.mincas.and.iz3.eq.1.and.
     &     i4.eq.101.and.iz4.eq.2) then
          call aqm(minsig,-106,sigaqm1,dummy)
          call aqm(1,106,sigaqm2,dummy)
          sig=3.02143713508091d-2/(e-1.66789841701854d0)**2
     &       *sigaqm1/sigaqm2
        elseif(iz1.eq.2.and.iz2.eq.-1.and.           !Sig+ + K-  -> Xi0 + pi0
     &    ((i3.eq.mincas.and.iz3.eq.1.and.               !           -> Xi- + pi+
     &      i4.eq.101.and.iz4.eq.0).or.
     &     (i3.eq.mincas.and.iz3.eq.-1.and.
     &      i4.eq.101.and.iz4.eq.2))) then
          sig=STREXCHANGE(e,minsig,2,-106,1,mincas,1,101,2)
     ...
     &     ((i3.eq.mincas.and.iz3.eq.1.and.              !           -> Xi- + pi0
     &       i4.eq.101.and.iz4.eq.-2).or.
     &      (i3.eq.mincas.and.iz3.eq.-1.and.
     &       i4.eq.101.and.iz4.eq.0))) then
          sig=STREXCHANGE(e,minsig,2,-106,1,mincas,1,101,2)
        elseif(iz1.eq.-2.and.iz2.eq.1.and.           !Sig- + aK0 -> Xi- + pi0
     &     ((i3.eq.mincas.and.iz3.eq.-1.and.             !           -> Xi0 + pi-
     &       i4.eq.101.and.iz4.eq.0).or.
     &      (i3.eq.mincas.and.iz3.eq.1.and.
     &       i4.eq.101.and.iz4.eq.-2))) then
          sig=STREXCHANGE(e,minsig,2,-106,1,mincas,1,101,2)
...
c K + aSigma
     ...
     &     i3.eq.-mincas.and.iz3.eq.-1.and.
     &     i4.eq.101.and.iz4.eq.-2) then
          call aqm(-mincas,106,sigaqm1,dummy)
          call aqm(1,106,sigaqm2,dummy)
          sig=3.02143713508091d-2/(e-1.66789841701854d0)**2
     &       *sigaqm1/sigaqm2
        elseif(iz1.eq.-2.and.iz2.eq.1.and.           !aSig- + K+ -> aXi0 + pi0
     &     ((i3.eq.-mincas.and.iz3.eq.-1.and.            !           -> aXi+ + pi-
     &       i4.eq.101.and.iz4.eq.0).or.
     &      (i3.eq.-mincas.and.iz3.eq.1.and.
     &       i4.eq.101.and.iz4.eq.-2))) then
          sig=STREXCHANGE(e,-minsig,-2,106,-1,-mincas,-1,101,-2)
     ...
     &     ((i3.eq.-mincas.and.iz3.eq.-1.and.            !           -> aXi+ + pi0
     &       i4.eq.101.and.iz4.eq.2).or.
     &      (i3.eq.-mincas.and.iz3.eq.1.and.
     &       i4.eq.101.and.iz4.eq.0))) then
          sig=STREXCHANGE(e,-minsig,-2,106,-1,-mincas,-1,101,-2)
        elseif(iz1.eq.2.and.iz2.eq.-1.and.           !aSig+ + K0 -> aXi+ + pi0
     &     ((i3.eq.-mincas.and.iz3.eq.1.and.             !           -> aXi0 + pi+
     &       i4.eq.101.and.iz4.eq.0).or.
     &      (i3.eq.-mincas.and.iz3.eq.-1.and.
     &       i4.eq.101.and.iz4.eq.2))) then
          sig=STREXCHANGE(e,-minsig,-2,106,-1,-mincas,-1,101,-2)
...
c aK + Lambda
      else if(i1.eq.minlam.and.i2.eq.-106) then
        if(iz1.eq.0.and.iz2.eq.1.and.                !Lam0 + aK0 -> Xi0 + pi0
     &    ((i3.eq.mincas.and.iz3.eq.1.and.               !           -> Xi- + pi+
     &      i4.eq.101.and.iz4.eq.0).or.
     &     (i3.eq.mincas.and.iz3.eq.-1.and.
     &      i4.eq.101.and.iz4.eq.2))) then
          call aqm(-minlam,-106,sigaqm1,dummy)
          call aqm(1,106,sigaqm2,dummy)
     ...
     &      (i3.eq.mincas.and.iz3.eq.-1.and.
     &       i4.eq.101.and.iz4.eq.0))) then
          sig=STREXCHANGE(e,minlam,0,-106,1,mincas,1,101,0)
        endif
c K + aLambda
      else if(i1.eq.-minlam.and.i2.eq.106) then
     ...
     &      (i3.eq.-mincas.and.iz3.eq.1.and.
     &       i4.eq.101.and.iz4.eq.-2))) then
          sig=STREXCHANGE(e,-minlam,0,106,1,-mincas,-1,101,2)
        endif
...
c Xi0 + K- -> Omega- + pi0
     ...
     &   i3.eq.minome.and.iz3.eq.0.and.
     &   i4.eq.101.and.iz4.eq.0) then
        sig=STREXCHANGE(e,mincas,1,-106,1,minome,0,101,2)
c Xi- + aK0 -> Omega- + pi0
      elseif(i1.eq.mincas.and.iz1.eq.-1.and.
     &   i2.eq.-106.and.iz2.eq.1.and.
     &   i3.eq.minome.and.iz3.eq.0.and.
...
c Lambda + Lambda 
c aLambda + aLambda
      elseif(abs(i1).eq.minlam.and.abs(i2).eq.minlam.and.
     &       i1*i2.gt.0) then
         if((e-massit(mincas)-massit(1)).le.0) then
            sig=0d0
         else
            sig=37.15d0*(e-massit(mincas)-massit(1))**(-0.16)
     &           *pcms(e,massit(mincas),massit(1))
     $           /pcms(e,massit(minlam),massit(minlam))
     &           /2d0
         endif
c Lambda0 + Sigma-  -> Xi- + n
c Lambda0 + Sigma+  -> Xi0 + p
c aLambda0 + aSigma+  -> aXi+ + an
c aLambda0 + aSigma-  -> aXi0 + ap
      elseif( (((abs(i1).eq.minlam.and.abs(i2).eq.minsig).and.
     &          (iz1.eq.0.and.abs(iz2).eq.2)).or.
     &         ((abs(i1).eq.minsig.and.abs(i2).eq.minlam).and.
     &          (abs(iz1).eq.2.and.iz2.eq.0))).and.
     &         i1*i2.gt.0 ) then
        if((e-massit(mincas)-massit(minlam)).le.0) then
          sig=0d0
        else
          sig=24.3781d0*(e-massit(minsig)-massit(minlam))**(-0.479d0)
        endif 
c Lambda0 + Sigma0  -> Xi0 + n
c                   -> Xi- + p
c aLambda0 + aSigma0  -> aXi0 + an
c                     -> aXi+ + ap
      elseif( ((abs(i1).eq.minlam.and.abs(i2).eq.minsig).or.
     &         (abs(i1).eq.minsig.and.abs(i2).eq.minlam)).and.
     &         i1*i2.gt.0.and.iz1.eq.0.and.iz2.eq.0) then
        if((e-massit(minsig)-massit(minlam)).le.0) then
          sig=0d0
        elseif((e-massit(minsig)-massit(minlam)).le.3.336d-2) then
          sig=6.475d0*(e-massit(minsig)-massit(minlam))**(-0.4167d0)
        else
          sig=14.5054d0*(e-massit(minsig)-massit(minlam))**(-0.1795d0)
        endif
c Sigma0  + Sigma0  -> Xi0 + n
c                   -> Xi- + p
c aSigma0  + aSigma0  -> aXi0 + an
c                     -> aXi+ + ap
      elseif(abs(i1).eq.minsig.and.abs(i2).eq.minsig.and.
     &       iz1.eq.0.and.iz2.eq.0.and.i1*i2.gt.0) then  
        if((e-2*massit(minsig)).le.0) then
          sig=0d0
        elseif((e-2*massit(minsig)).le.9.047d-2) then
          sig=5.625d0*(e-2*massit(minsig))**(-0.318d0)
        else
          sig=4.174d0*(e-2*massit(minsig))**(-0.4421d0)
        endif
c Sigma0  + Sigma+  -> Xi0 + p
c Sigma0  + Sigma-  -> Xi- + n
c aSigma0  + aSigma-  -> aXi0 + ap
c aSigma0  + aSigma+  -> aXi+ + an
     ...
     &        (abs(iz1).eq.2.and.iz2.eq.0)).and.
     &       i1*i2.gt.0) then
        sig=4d0*STREXCHANGE(e,minsig,0,minsig,0,mincas,-1,1,1)
c Sigma+  + Sigma-  -> Xi0 + n
c                   -> Xi- + p
c aSigma-  + aSigma+  -> aXi0 + an
c                     -> aXi+ + ap
     ...
     &       i1*i2.gt.0.and.(iz1+iz2).eq.0) then
        if((e-2*massit(minsig)).le.0) then
          ...
          sig=14.194*(e-2*massit(minsig))**(-0.442)
        endif
c Xi0 + n -> Lambda0 + Lambda0
c Xi0 + n -> Lambda0 + Sigma0
c Xi0 + n -> Sigma0  + Sigma0
...
c K- + p -> Y + pi
        if(i1.eq.1.and.iz1.eq.1.and.        !proton
     &     i2.eq.-106.and.iz2.eq.-1) then   !K-
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,101,-2) !Sigma+ + Pi-
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,minsig,-2,101,2) !Sigma- + Pi+
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,0)  !Lambda0/Sigma0 + Pi0
c K- + n -> Y + pi
        else if (i1.eq.1.and.iz1.eq.-1.and.  !neutron
     &     i2.eq.-106.and.iz2.eq.-1) then    !K-
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,minsig,-2,101,0) !Sigma- + Pi0
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,-2) !Lambda0/Sigma0 + Pi-
c K+ + ap -> aY + pi
        else if (i1.eq.-1.and.iz1.eq.-1.and. !aProton
     &     i2.eq.106.and.iz2.eq.1) then      !K+
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,2,101,-2) !aSigma+ + Pi-
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,-2,101,2) !aSigma- + Pi+
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,101,0)  !aLambda0/aSigma0 + Pi0
c K+ + an -> aY + pi
        else if (i1.eq.-1.and.iz1.eq.1.and.  !aNeutron
     &     i2.eq.106.and.iz2.eq.1) then      !K+
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,2,101,0)  !aSigma+ + Pi0
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,101,2)  !aLambda0/aSigma0 + Pi+
c K0 + ap
        else if (i1.eq.-1.and.iz1.eq.-1.and. !aProton
     &     i2.eq.106.and.iz2.eq.-1)  then    !K0
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,-2,101,0) !aSigma- + Pi0
...
c aK0 + p
     ...
     &     i2.eq.-106.and.iz2.eq.1)  then    !aK0
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,101,0) !Sigma+ + Pi0
     &       + STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,101,2) !Lambda0/aSigma0 + Pi+
c aK0 + n
        else if (i1.eq.1.and.iz1.eq.-1.and.  !neutron
     &     i2.eq.-106.and.iz2.eq.1)  then    !aK0
          sig= STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,101,-2) !Sigma+ + Pi-
...
c Lambda0 + pi0 -> K-  +p
c Lambda0 + pi0 -> aK0 +n
        elseif(i1.eq.minlam.and.iz1.eq.0.and.   !aLambda
     &     i2.eq.101.and.iz2.eq.0)  then    !pi0
           sig= (STREXCHANGE(e,1,1,-106,-1,i1,iz1,i2,iz2)
     &          *pcms(e,massit(-106),massit(-1))**2
     &          /pcms(e,massit(i1),massit(i2))**2
...
c Sigma0 + pi+  -> aK0 + p
     ...
     &        * CTParam(72)
c Lambda0 + pi+ -> aK0 + p
        elseif(i1.eq.minlam.and.iz1.eq.0.and.  !Lambda
     &     i2.eq.101.and.iz2.eq.2)  then   !pi+
           sig= (STREXCHANGE(e,1,1,-106,1,i1,iz1,i2,iz2)
     &          *pcms(e,massit(-106),massit(1))**2
     &          /pcms(e,massit(i1),massit(i2))**2)
...
c Sigma+ + K- -> Xi- + pi+
        if(i1.eq.minsig.and.iz1.eq.2.and.                     !Sigma+
     &     i2.eq.-106.and.iz2.eq.-1) then                 !K-
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,0) 
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,101,2) 
c Sigma+ + aK0-> Xi0 + pi+
        else if(i1.eq.minsig.and.iz1.eq.2.and.                !Sigma+
     &     i2.eq.-106.and.iz2.eq.1) then                  !aK0
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,2) 
c Lambda0/Sigma0 + aK0 -> Xi0 + pi0
c Lambda0/Sigma0 + aK0 -> Xi- + pi+
        else if((i1.eq.minsig.or.i1.eq.minlam).and.iz1.eq.0.and.  !Lambda0/Sigma0
     &     i2.eq.-106.and.iz2.eq.1) then                  !aK0
...
c Lambda0/Sigma0 + K-  -> Xi- + pi0
     ...
     &     i2.eq.-106.and.iz2.eq.-1) then                 !aK-
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,101,-2)
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,101,0)
c Sigma- + aK0 -> Xi- + pi0
c Sigma- + aK0 -> Xi0 + pi-
        else if(i1.eq.minsig.and.iz1.eq.-2.and.               !Sigma-
     &     i2.eq.-106.and.iz2.eq.1) then                  !aK0
...
c Xi0 + pi0 -> aK0+ Lambda0/Sigma0
        if(i1.eq.mincas.and.iz1.eq.1.and.        !Xi0
     &     i2.eq.101.and.iz2.eq.0) then      !pi0
          sig =STREXCHANGE(e,minsig,2,-106,-1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minsig))**2
     &        /pcms(e,massit(i1),massit(i2))**2
     &        +STREXCHANGE(e,minsig,0,-106,1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minsig))**2
     &        /pcms(e,massit(i1),massit(i2))**2
     &        +STREXCHANGE(e,minlam,0,-106,1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minlam))**2
     &        /pcms(e,massit(i1),massit(i2))**2
c Xi0 + pi- -> K- + Lambda0/Sigma0
c Xi0 + pi- -> aK0+ Sigma-
        elseif(i1.eq.mincas.and.iz1.eq.1.and.    !Xi0
     &     i2.eq.101.and.iz2.eq.-2) then     !pi-
          sig =STREXCHANGE(e,minsig,0,-106,-1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minsig))**2
     &        /pcms(e,massit(i1),massit(i2))**2
     &        +STREXCHANGE(e,minlam,0,-106,-1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minlam))**2
     &        /pcms(e,massit(i1),massit(i2))**2
     &        +STREXCHANGE(e,minsig,-2,-106,1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(minsig))**2
     &        /pcms(e,massit(i1),massit(i2))**2
c Xi0 + pi+ -> aK0+ Sigma+
        elseif(i1.eq.mincas.and.iz1.eq.1.and.    !Xi0
...
c Xi0 + aK0 -> Omega- + pi+
        if(i1.eq.mincas.and.iz1.eq.1.and.
     &     i2.eq.-106.and.iz2.eq.1) then
          sig =STREXCHANGE(e,i1,iz1,i2,iz2,minome,0,101,2)
c Xi0 + K- -> Omega- + pi0
        elseif(i1.eq.mincas.and.iz1.eq.1.and.
     &     i2.eq.-106.and.iz2.eq.-1) then
          sig =STREXCHANGE(e,i1,iz1,i2,iz2,minome,0,101,0)
c Xi- + aK0 -> Omega- + pi0
        elseif(i1.eq.mincas.and.iz1.eq.-1.and.
     &     i2.eq.-106.and.iz2.eq.1) then
          sig =STREXCHANGE(e,i1,iz1,i2,iz2,minome,0,101,0)
...
c Omega- + pi+ -> Xi0 + aK0
        if(i1.eq.minome.and.iz1.eq.0.and.
     &     i2.eq.101.and.iz2.eq.2) then
          sig=5d-1*STREXCHANGE(e,mincas,1,-106,1,i1,iz1,i2,iz2)
     &        *pcms(e,massit(-106),massit(mincas))**2
     &        /pcms(e,massit(i1),massit(i2))**2
c Omega- + pi0 -> Xi0 + K-
c Omega- + pi0 -> Xi- + aK0
     ...
     &     i2.eq.101.and.iz2.eq.0) then
          sig=5d-1*(STREXCHANGE(e,mincas,1,-106,-1,i1,iz1,i2,iz2)
     &             +STREXCHANGE(e,mincas,-1,-106,1,i1,iz1,i2,iz2))
     &        *pcms(e,massit(-106),massit(mincas))**2
     &        /pcms(e,massit(i1),massit(i2))**2
c Omega- + pi- -> Xi- + K-
        elseif(i1.eq.minome.and.iz1.eq.0.and.
...
c aLambda0 + aLambda0 -> aXi0 + an
c                     -> aXi+ + ap
        if(abs(i1).eq.minlam.and.abs(i2).eq.minlam.and.
     &     i1*i2.gt.0) then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,1,-1)
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,1,1)
c Lambda0 + Sigma-  -> Xi- + n
c Lambda0 + Sigma+  -> Xi0 + p
c aLambda0 + aSigma+  -> aXi+ + an
c aLambda0 + aSigma-  -> aXi0 + ap
...
c aLambda0 + aSigma0  -> aXi0 + an
c                     -> aXi+ + ap
        elseif( ((abs(i1).eq.minlam.and.abs(i2).eq.mincas).or.
     &           (abs(i1).eq.mincas.and.abs(i2).eq.minlam)).and.
     &           i1*i2.gt.0.and.iz1.eq.0.and.iz2.eq.0) then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,1,1,-1)
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,1,1)
c Sigma0  + Sigma0  -> Xi0 + n
...
c                     -> aXi+ + ap
     ...
     &         abs(iz1).eq.2.and.abs(iz2).eq.2.and.
     &         i1*i2.gt.0.and.(iz1+iz2).eq.0) then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,mincas,0,1,-1)
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,mincas,-1,1,1)
        endif
...
c Xi- + p -> Sigma0  + Sigma0
c Xi- + p -> Sigma+  + Sigma-
        if( (i1.eq.mincas.and.iz1.eq.1.and.i2.eq.1.and.iz2.eq.-1).or.
     &      (i1.eq.1.and.iz1.eq.-1.and.i2.eq.mincas.and.iz2.eq.1).or.
     &      (i1.eq.mincas.and.iz1.eq.-1.and.i2.eq.1.and.iz2.eq.1).or.
     &      (i1.eq.1.and.iz1.eq.1.and.i2.eq.mincas.and.iz2.eq.-1)) then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,minlam,0,minlam,0) !Lam + Lam
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,minlam,0,minsig,0) !Lam + Sig0
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,minsig,0) !Sig0 + Sig0
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,minsig,2,minsig,-2) !Sig+ + Sig-

c Xi0 + p -> Lambda0 + Sigma+
c Xi0 + p -> Sigma0  + Sigma+
        elseif((i1.eq.mincas.and.iz1.eq.1.and.i2.eq.1.and.iz2.eq.1).or.
     &        (i1.eq.1.and.iz1.eq.1.and.i2.eq.mincas.and.iz2.eq.1)) then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,minlam,0,minsig,2) !Lam + Sig+
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,minsig,0,minsig,2) !Sig0+ Sig+
c Xi- + n -> Lambda0 + Sigma-
c Xi- + n -> Sigma0  + Sigma-
       elseif((i1.eq.mincas.and.iz1.eq.-1.and.i2.eq.1.and.iz2.eq.-1).or.
     &      (i1.eq.1.and.iz1.eq.-1.and.i2.eq.mincas.and.iz2.eq.-1)) then
...
c aXi- + ap -> aSigma+  + aSigma-
     ...
     &       (i1.eq.-mincas.and.iz1.eq.1.and.i2.eq.-1.and.iz2.eq.-1).or.
     &      (i1.eq.-1.and.iz1.eq.-1.and.i2.eq.-mincas.and.iz2.eq.1))then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,-minlam,0,-minlam,0) !aLam + aLam
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,-minlam,0,-minsig,0) !aLam + aSig0
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,-minsig,0) !aSig0 + aSig0
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,-2,-minsig,2) !aSig- + aSig+

c aXi0 + ap -> aLambda0 + aSigma-
c aXi0 + ap -> aSigma0  + aSigma-
      elseif((i1.eq.-mincas.and.iz1.eq.-1.and.i2.eq.-1.and.iz2.eq.-1)
     $         .or.
     &    (i1.eq.-1.and.iz1.eq.-1.and.i2.eq.-mincas.and.iz2.eq.-1))then
          sig=STREXCHANGE(e,i1,iz1,i2,iz2,-minlam,0,-minsig,-2) !aLam + aSig-
     &       +STREXCHANGE(e,i1,iz1,i2,iz2,-minsig,0,-minsig,-2) !aSig0+ aSig-
