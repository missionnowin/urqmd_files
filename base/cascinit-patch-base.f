...
c               write (6,*) "rejected: ",R
     ...
     &             +(PT_rz(j,nucleus)-PT_rz(k,nucleus))**2
              if (drr.lt.mindist/CTParam(67)**(1./3.).and.
...
c         if(eos.eq.0)then
c            PT_fmass(i,nucleus)=sqrt(0.938**2-p**2)
c         endif
...
c injecting fluctons with spatial distribution  
      if (nucleus .eq. 1) then  ! Projectile nucleus
         fluction_max_n = 7
      elseif(nucleus .eq. 2) then  ! Target nucleus
         fluction_max_n = 7
      else 
         fluction_max_n = 0
      end if
...
c Initialize pairing array
      do ii = 1, 300
         is_fluction_paired(ii) = .false.
      end do
      
c Create up to fluction_max_n fluctions with spatial separation
      do kk = 1, fluction_max_n
         
c Find first unpaired nucleon
         do ii = 1, Z
            if (.not. is_fluction_paired(ii)) then
               
c Try up to 20 times to find a distant partner
               do att_cnt = 1, 20
                  randd = ranf(0)
                  jj = 1 + int(randd * Z)
                  
c Accept if: not same, not paired, and far apart (>5 indices)
                  if (ii.ne.jj .and. .not. is_fluction_paired(jj) .and.
     &               abs(ii-jj).le.5) then
                     
c Create fluction: combine nucleon ii with nucleon jj                  
                     PT_pz(jj,nucleus)=PT_pz(jj,nucleus)
     &                                 +PT_pz(ii,nucleus)
                     PT_p0(jj,nucleus)=sqrt(PT_px(jj,nucleus)**2
     &                                 +PT_py(jj,nucleus)**2
     &                                 +PT_pz(jj,nucleus)**2
     &                                 +PT_fmass(jj,nucleus)**2)
                     
c Remove nucleon ii (original formula)
                     PT_pz(ii,nucleus)=0
                     PT_p0(ii,nucleus)=sqrt(PT_px(ii,nucleus)**2
     &                                 +PT_py(ii,nucleus)**2
     &                                 +PT_pz(ii,nucleus)**2
     &                                 +PT_fmass(ii,nucleus)**2)
                     PT_rx(ii,nucleus)=-10000
                     
c Mark both as paired
               ...
               if (fluction_cnt .ge. fluction_max_n) exit
            end if
         end do
     ...
     &             ' spatially distributed fluctions in nucleus'
      end if
...
c      beta = pin/sqrt(pin**2+0.938d0**2)
